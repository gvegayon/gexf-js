...
function parseGEXF(data, obj, isJson) {
  ...
  if (isJson) {
      ...
      _graph = $(data).find("graph");
      _nodes = _graph.children().filter("nodes").children();
      _edges = _graph.children().filter("edges").children();
      
      obj.graph = {
          directed: (_graph.attr("defaultedgetype") == "directed"),
          nodeList: [],
          indexOfLabels: [],
          edgeList: [],
          attributes: {},
      ...
      $(_edges).each(function () {
          ...
          if (_col.length) {
              ...
              if (_directed) {
                  ...
                  _tcol = obj.graph.nodeList[_tix].rgb;
                  _r = Math.floor(0.5 * _scol[0] + 0.5 * _tcol[0]);
                  _g = Math.floor(0.5 * _scol[1] + 0.5 * _tcol[1]);
                  _b = Math.floor(0.5 * _scol[2] + 0.5 * _tcol[2]);
              }
          }
...
function GEXFjs(obj) {
  ...
  // Checking undefined
  var d;
  for (d in this.GEXFjsDefaults) {
    if (typeof this[d] === "undefined") {
      this[d] = this.GEXFjsDefaults[d];
    }
  }
  
  for (d in this.GEXFjsDefaultParams) {
    if (typeof this.params[d] === "undefined") {
      this.params[d] = this.GEXFjsDefaultParams[d];
    }
  }
  ...
  this.lang = (this.DICTIONARY[this.lang] ? this.lang : "en");
  
  this.strLang = (function(_str) {
    return this.DICTIONARY[this.lang][_str.replace("_", " ")];
  });
  ...
  this.setLanguage = (function(lan) {
    ...
    if (typeof this.DICTIONARY[lan] === "undefined") {
      window.alert(
...
function renderGEXF(GexfJS) {
    ...
    function loadGraph() {
        ...
        if (typeof GexfJS.params.graphString !== "undefined") {
          
          var parser = new DOMParser();
          var data = parser.parseFromString(GexfJS.params.graphString, "text/xml");
    ...
    function traceMap() {
        ...
        GexfJS.params.mousePosition = (
          GexfJS.params.useLens ?
          (GexfJS.mousePosition ? (GexfJS.mousePosition.x + "," + GexfJS.mousePosition.y) : "out") :
          null
          );
        ...
        var _sizeFactor = GexfJS.globalScale * Math.pow(GexfJS.globalScale, -0.15),
        ...
        for (i in GexfJS.graph.nodeList) {
            ...
            _d.withinFrame = (
              (_d.actual_coords.x + _d.actual_coords.r > 0) &&
              (_d.actual_coords.x - _d.actual_coords.r < GexfJS.graphZone.width) &&
              (_d.actual_coords.y + _d.actual_coords.r > 0) &&
              (_d.actual_coords.y - _d.actual_coords.r < GexfJS.graphZone.height)
              );
        ...
        if (!GexfJS.params.isMoving && (GexfJS.params.showEdges || _centralNode != -1)) {
            ...
            for (i in GexfJS.graph.edgeList) {
              
                _d = GexfJS.graph.edgeList[i];
                _six = _d.s;
                _tix = _d.t;
                ...
                if ((_isLinked || _showAllEdges) && (_ds.withinFrame || _dt.withinFrame) && _ds.visible && _dt.visible) {
                    ...
                    _coords = (
                      (GexfJS.params.useLens && GexfJS.mousePosition) ?
                      calcCoord(GexfJS.mousePosition.x, GexfJS.mousePosition.y, _ds.actual_coords) :
                      _ds.actual_coords
                      );
                    ...
                    _coordt = (
                      (GexfJS.params.useLens && GexfJS.mousePosition) ?
                      calcCoord(
                        GexfJS.mousePosition.x,
                        GexfJS.mousePosition.y,
                        _dt.actual_coords
                        ) :
                      _dt.actual_coords
                      );
                      
                    GexfJS.ctxGraphe.strokeStyle = (
                      _isLinked ? _d.C : "rgba(100,100,100,0.2)"
                      );
                    
                    traceArc(
                      GexfJS.ctxGraphe, _coords, _coordt, _sizeFactor * 3.5,
                      GexfJS.params.showEdgeArrow && _d.d
                      );
                }
            }
        ...
        if (_centralNode != -1) {
            ...
            _dnc = GexfJS.graph.nodeList[_centralNode];
        ...
        for (i in GexfJS.graph.nodeList) {
            ...
            if (_d.visible && _d.withinFrame) {
                if (i != _centralNode) {
                    ...
                    _d.real_coords = (
                      (GexfJS.params.useLens && GexfJS.mousePosition) ?
                      calcCoord(GexfJS.mousePosition.x, GexfJS.mousePosition.y, _d.actual_coords) :
                      _d.actual_coords);
        ...
        for (i in GexfJS.graph.nodeList) {
            ...
            if (_d.visible && _d.withinFrame) {
                if (i != _centralNode) {
                    ...
                    _fs = _d.real_coords.r * _textSizeFactor;
                    
                    if (_d.isTag) {
                        if (_centralNode != -1) {
                            _dist = Math.sqrt(Math.pow(_d.real_coords.x - _dnc.real_coords.x, 2) + Math.pow(_d.real_coords.y - _dnc.real_coords.y, 2));
                            if (_dist > 80) {
                    ...
                    if (_fs > GexfJS.params.textDisplayThreshold) {
                        ...
                        GexfJS.ctxGraphe.fillStyle = (
                          (i != GexfJS.params.activeNode) &&
                          _tagsMisEnValeur.length &&
                          ((!_d.isTag) || (_centralNode != -1)) ?
                          "rgba(60,60,60,0.7)" :
                          "rgb(0,0,0)"
                          );
    ...
    /**
     * Where all the action happens. This writes down the functions on each bit
     * of the document.
     */ 
